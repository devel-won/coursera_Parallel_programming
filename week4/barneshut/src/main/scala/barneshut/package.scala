...
package object barneshut {
  ...
  case class Fork(
    ...
    val massX: Float = if (mass != 0) quads.map(q => q.mass * q.massX).sum / mass else centerX
    val massY: Float = if (mass != 0) quads.map(q => q.mass * q.massY).sum / mass else centerY
    val total: Int = quads.map(_.total).sum

    def insert(b: Body): Fork = {
      val xFlag = b.x < centerX
      val yFlag = b.y < centerY

      if (xFlag && yFlag) Fork(nw.insert(b), ne, sw, se)
      else if (xFlag && !yFlag) Fork(nw, ne.insert(b), sw, se)
      else if (!xFlag && yFlag) Fork(nw, ne, sw.insert(b), se)
  ...
  extends Quad {
    ...
    def insert(b: Body): Quad = {
      ...
      if (minimumSize > size) {
        ...
        addBodies.foldLeft(Fork(nw, ne, sw, se))((a, b) => a.insert(b))
      }
      else
        copy(bodies = addBodies)
    }
  }
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) extends SectorMatrixInterface {
    ...
    def +=(b: Body): SectorMatrix = {
      val xMin = math.min(math.max(boundaries.minX, b.x), boundaries.maxX)
      val yMin = math.min(math.max(boundaries.minY, b.y), boundaries.maxY)
      apply(((xMin - boundaries.minX) / sectorSize).toInt, ((yMin - boundaries.minY) / sectorSize).toInt) += b
      this
    }
